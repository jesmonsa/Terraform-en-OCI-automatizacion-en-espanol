...
resource "null_resource" "produccionWebserver1HTTPD" {
  depends_on = [oci_core_instance.produccionWebserver1]
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "opc"
      host        = data.oci_core_vnic.produccionWebserver1_VNIC1.public_ip_address
...
resource "null_resource" "produccionWebserver2HTTPD" {
  depends_on = [oci_core_instance.produccionWebserver2]
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "opc"
      host        = data.oci_core_vnic.produccionWebserver2_VNIC1.public_ip_address
      private_key = tls_private_key.public_private_key_pair.private_key_pem
      script_path = "/home/opc/myssh.sh"
      agent       = false
      timeout     = "10m"
